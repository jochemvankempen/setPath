function decode_eachElectrodeEachTimeEachFreq(data, savefilename, trialLabel,decodeSPGDir,times2downsample,freq,DP,C,nCond)
lastwarn('');
warning('on','all')
...
for iTime = 1:DP.nTime
    tmp4=[];
    for iFreq = 1:DP.nFreq
        if isNotEnoughTrials
            return
        end
        %%
        [tmp4 DP] = decode_extractFeatures(DP,data,iTime,iFreq);
        ...
        [P{iTime,iFreq}] = decodeRLSC(cResp,C,DP);
        ...
        if DP.permute
            parfor iPerm = 1:DP.nPermutes
                ...
                [Pperm{iTime,iFreq,iPerm}] = decodeRLSC(cRespPerm,C,DP);
...
for iTime = 1:DP.nTime
    for iFreq = 1:DP.nFreq
        for iLambda = 1:C.nLam
            R.zmcorrTrain(iTime,iFreq,:,iLambda) = [P{iTime,iFreq}(iLambda,:).zmcorrTrain];
            R.zmcorrTest(iTime,iFreq,:,iLambda) = [P{iTime,iFreq}(iLambda,:).zmcorrTest];
            
            if DP.permute
                for iPerm = 1:DP.nPermutes
                    Rperm.zmcorrTrain(iTime,iFreq,:,iLambda,iPerm) = [Pperm{iTime,iFreq,iPerm}(iLambda,:).zmcorrTrain];
                    Rperm.zmcorrTest(iTime,iFreq,:,iLambda,iPerm) = [Pperm{iTime,iFreq,iPerm}(iLambda,:).zmcorrTest];
                end
            end
        end
    end
