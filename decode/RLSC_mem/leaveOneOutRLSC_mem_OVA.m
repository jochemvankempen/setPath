function [ind_train,ind_test] = leaveOneOutRLSC_mem_OVA(cResp,C,trialnum)
% 07 Mar 17
    ...
    elseif C.randomSampleTest == 0
        ...
        if 0
            for iClass = 1:length(cResp)
                ...
                tmp{iClass,1} = [1:(nTrialPerSession(iClass)); repmat(iClass,1,nTrialPerSession(iClass))]';
                tmp{iClass,2} = [];
                for iClassCompare = classAll
                    tmp{iClass,2} = cat(1,tmp{iClass,2},[1:nTrialPerSession(iClassCompare); repmat(iClassCompare,1,nTrialPerSession(iClassCompare))]');
                end
            end
            ...
            for iSample = 1:C.nFoldValidation
                for iClass = 1:length(cResp)
                    tmpRand{1} = randperm(length(tmp{iClass,1}));
                    tmpRand{2} = randperm(length(tmp{iClass,2}));
                    
                    
                    
                    ind_test(iSample,iClass,1,:,:)=   tmp{iClass,1}(tmpRand{1}(1:nTest),:);
                    [~,sortIdx] = sort(tmp{iClass,2}(tmpRand{2}(1:nTest),2));
                    %                 ind_test(iSample,iClass,2,:,:)=   (tmp{iClass,2}(tmpRand{2}(1:nTest),:));
                    ind_test(iSample,iClass,2,:,:)=   (tmp{iClass,2}(tmpRand{2}(sortIdx),:));
                    
                    ind_train(iSample,iClass,1,:,:)=   tmp{iClass,1}(tmpRand{1}((nTest+1):(nTest+nTrain)),:);
                    permIdx = tmpRand{2}((nTest+1):(nTest+nTrain));
                    [~,sortIdx] = sort(tmp{iClass,2}(permIdx,2));
                    ind_train(iSample,iClass,2,:,:)=   tmp{iClass,2}(permIdx(sortIdx),:);
                    
                    %
                    %                 ind_train(iSample,iClass,1,:)=  tmp((nTest+1):(nTest+nTrain));
                    %                 ind_train(iSample,iClass,2,:)=  tmp((nTest+1):(nTest+nTrain));
                end
            ...
            for iSample = 1:C.nFoldValidation
                tmp = randperm(nTrialPerSession
                ind_test(iSample,:)=   tmp{iClass,1}(tmpRand{1}(1:nTest),:);
