function [ind_train,ind_test] = leaveOneOutRLSC_mem(cResp,C,trialnum)
% 07 Mar 17
...
if ~isfield(C,'nCrossVal')
    C.nCrossVal = nSession;
end
...
if 0
    ...
    for iClass = 1:length(cResp)
        ...
        nTrialPerValidation(iClass) = nTrialPerSession(iClass)/C.nCrossVal;
    end
    for iClass = 1:length(cResp)
...
if isempty(C.randomSampleTest)
    ...
    for iClass = 1:length(cResp)
        nTrialPerSession(iClass) = size(cResp{iClass},1);
        nCeiledTrialPerValidation(iClass) = ceil(nTrialPerSession(iClass)/C.nCrossVal);
        nFlooredSession(iClass) = nCeiledTrialPerValidation(iClass)*C.nCrossVal -  nTrialPerSession(iClass) ;
    end

    jEndLastTestTrial = zeros(length(cResp));
    for iSample = 1:C.nCrossVal
        for iClass = 1:length(cResp)
            if iSample <= nFlooredSession(iClass)
                nTrialPerValidation(iClass,iSample) = floor(nTrialPerSession(iClass)/C.nCrossVal);
            else
                nTrialPerValidation(iClass,iSample) = ceil(nTrialPerSession(iClass)/C.nCrossVal);
            end
        end
    ...
    elseif C.randomSampleTest == 0
        ...
        if 1
            ...
            C.nCrossVal = size(cResp{1},1) + size(cResp{2},1);
            ...
            C.nCrossVal = min(nTrialPerSession);

            if nTrain + nTest > C.nCrossVal
            ...
            for iSample = 1:C.nCrossVal
                for iClass = 1:length(cResp)
                    ...
                    elseif iSample + C.nHalfTraining > C.nCrossVal
                        vTrainTrial{iSample,iClass} = [(C.nCrossVal - 2*C.nHalfTraining ):(iSample-1) ,...
                            (iSample+1):(C.nCrossVal)] ;
                    else
