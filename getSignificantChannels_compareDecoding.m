function [signChannels signDim signMW decodeChan times2downsample signChannelsAllDimAllTW thresholdSetting] = getSignificantChannels_TW(subID,arrayID,SPG,DP,EXP,DIR)
...
for iChan = (Channels2check(length(Channels2checkOriginal)+1:end))
    ...
    filename = ['dTF_' subID '_' val '_' DP.decodeSPG '_' EXP.cond '_' DP.ext];
    
    if ~exist([DIR.decodeCheck filesep filename '.mat'],'file')
        filename
        keyboard
        continue
    end
    decodeChan(iChan) = load([DIR.decodeCheck filesep filename '.mat']);
end
%%
                    ...
                    for iTTW =DP.nTTW
                        ...
                        switch thresholdSetting
                            ...
                            case 'threshold'
                                ...
                                if ~isempty(find(decodeChan(iChan).pMeanTestCorr(iDim,iTW,iTTW).TF>threshold))
                                    signChannels    (iChan)                                 = 1;
                                    signDim         (iChan,iDim)                            = 1;
                                    signMW          (iChan,iDim,iTW,iTTW)   = 1;
                                end
