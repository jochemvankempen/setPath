function [S,t,f,Serr]=mtspecgramc_phase(data,movingwin,params)
...
% edit Jochem van Kempen, 22-04-13
...
for n=1:nw;
   ...
   datawin=data(indx,:);
   if nargout==4
     [s,f,serr]=mtspectrumc_phase(datawin,params);
     Serr(1,n,:,:)=squeeze(serr(1,:,:));
     Serr(2,n,:,:)=squeeze(serr(2,:,:));
   else
     [s,f]=mtspectrumc_phase(datawin,params);
   end
   S(n,:,:)=s;
end;
S=squeeze(S); 
