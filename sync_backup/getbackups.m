function lists = getbackups(fileName, backupPath, begDate)
%GETBACKUPS get all backups of the file with filename under backupPath
%
% See also: backupfolder, revertbackup

% Copyright: zhang@zhiqiang.org

if nargin <= 2, begDate = '1990-01-01'; end
...
if fileName(1) == '\', fileName = fileName(2:end); end

lists = {};

files = sortstruct(dir(backupPath), 'datenum', -1);
for i = 1:numel(files)
    f = files(i);
    if strcmp(f.name, '.') || strcmp(f.name, '..') ...
            || isempty(regexp(f.name, '^\d\d\d\d\-\d\d\-\d\d$', 'once'))
        continue;
    end
    
    if exist([backupPath f.name '\' fileName], 'file')
        if datenum(f.name) >= datenum(begDate)
            lists = [lists; {[backupPath f.name '\' fileName]}]; %#ok<AGROW>
        end
    end
end
